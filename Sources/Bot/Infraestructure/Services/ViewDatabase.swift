...
class ViewDatabase {
    ...
    func open(path: String, user: Identity) throws {
        ...
        try db.transaction {
            if db.userVersion == 0 {
                ...
                let schemaV1url = Bundle.main.url(forResource: "ViewDatabaseSchema.sql", withExtension: nil)!
    ...
    func stats(for feed: FeedIdentifier) throws -> Int {
        ...
        do {
            ...
            Logger.shared.optional(SSBError.duringProcessing("stats for feed failed", error), nil)
            return 0
        }
    }
 ...
 //           .filter(colAboutID == aboutID)
    ...
    // MARK: recent
    func recentPosts(limit: Int, offset: Int? = nil, wantPrivate: Bool = false, onlyFollowed: Bool = true) throws -> [KeyValue] {
    ...
    private func addNumberOfPeopleReplied(msgs: [KeyValue]) throws -> [KeyValue] {
        var r: [KeyValue] = []
    ...
    func mentions(limit: Int = 200, wantPrivate: Bool = false, onlyImages: Bool = true) throws -> [KeyValue] {
        guard let _ = self.openDB else {
            throw ViewDatabaseError.notOpen
        }
    ...
    func feed(for identity: Identity, limit: Int = 5, offset: Int? = nil) throws -> [KeyValue] {
        guard let db = self.openDB else {
            throw ViewDatabaseError.notOpen
        }
        ...
        if try db.pluck(doWeBlockThemQry) != nil {
            throw SSBError.duringProcessing("user blocked", ViewDatabaseError.unknownAuthor(identity))
        }
...
//    func get(msgID: Int64) throws -> KeyValue {
        ...
        guard let msgType = typeMaybe else {
            Logger.shared.unexpected(.botError, "[viewdb] should have type for this message: \(msgId): \(key)")
        ...
        switch ct {
            case .post:
                ...
                if kv.count != 1 {
                    Logger.shared.unexpected(.botError, "[viewdb] could not find post after we had the type!?")
                    throw ViewDatabaseError.unknownMessage(key)
                }
                return kv[0]
    ...
    private func fillAddress(msgID: Int64, msg: KeyValue) throws {
        ...
        guard let a = msg.value.content.address else {
            Logger.shared.info("[viewdb/fill] broken addr message: \(msg.key)")
            return
        }
    ...
    private func fillAbout(msgID: Int64, msg: KeyValue) throws {
        ...
        guard let a = msg.value.content.about else {
            Logger.shared.info("[viewdb/fill] broken about message: \(msg.key)")
            return
        }
    ...
    private func fillContact(msgID: Int64, msg: KeyValue) throws {
        ...
        guard let c = msg.value.content.contact else {
            Logger.shared.info("[viewdb/fill] broken contact message: \(msg.key)")
            return
        }
        ...
        if b && f {
            Logger.shared.info("[viewdb/fill] broken contact. both b & f: \(msg.key)")
            return
        }
    ...
    private func checkAndExecuteDCR(msgID: Int64, msg: KeyValue) throws {
        ...
        guard let targetMsg = claimedMsg else {
            ...
            throw SSBError.unexpectedFault("dcr handling error: should have thrown already")
        }
    ...
    private func fillPub(msgID: Int64, msg: KeyValue) throws {
        ...
        guard let p = msg.value.content.pub else {
            Logger.shared.info("[viewdb/fill] broken pub message: \(msg.key)")
            return
        }
    ...
    private func fillPost(msgID: Int64, msg: KeyValue, pms: Bool) throws {
        ...
        guard let p = msg.value.content.post else {
            Logger.shared.info("[viewdb/fill] broken post message: \(msg.key)")
            return
        }
    ...
    private func fillVote(msgID: Int64, msg: KeyValue, pms: Bool) throws {
        ...
        guard let v = msg.value.content.vote else {
            Logger.shared.info("[viewdb/fill] broken vote message: \(msg.key)")
            return
        }
        ...
        try db.transaction { // also batches writes! helps a lot with perf
            ...
            for (msgIndex, msg) in msgs.enumerated() {
                if let msgRxSeq = msg.receivedSeq {
                    ...
                    if !pms {
                        throw SSBError.unexpectedFault("ViewDB: no receive sequence number on message")
                    }
                }
                ...
                if isOldMessage(msg: msg) && (msg.value.content.type != .contact && msg.value.content.type != .about)  {
                    ...
                    Analytics.shared.trackBotDidSkipMessage(key: msg.key, reason: "Old Message")
                    continue
                }
                ...
                if msgIndex%100 == 0 {
                    let done = Float64(msgIndex)/Float64(msgs.count)
                ...
                if pms {
                    ...
                    do {
                        ...
                        throw SSBError.unexpectedFault("ViewDB/INSERT message error \(errCode): \(errMsg)")
                    } catch {
                        throw error
                    }
                }
                ...
                do { // identifies which message failed
                    ...
                    throw SSBError.duringProcessing("fillMessages threw on msg(\(msg.key)", error)
                }
    ...
    private func msgID(of msg: KeyValue, make: Bool = false) throws -> Int64 {
        ...
        guard let hk = msg.hashedKey else { throw SSBError.unexpectedFault("missing hashed key on fresh message") }
        return try db.run(self.msgKeys.insert(
            colKey <- msg.key,
            colHashedKey <- hk
        ))
    ...
    private func insertPrivateRecps(msgID: Int64, recps: [RecipientElement]?) throws {
        ...
        guard let recps = recps else {
            ...
            Logger.shared.unexpected(.missingValue, "[viewdb] warning: no recps in private message (msgID:\(msgID))")
            return
        }
    ...
    func numberOfMessages(for feed: FeedIdentifier) throws -> Int {
        ...
        do {
            ...
            Logger.shared.optional(SSBError.duringProcessing("numberOfmessages for feed failed", error))
            return 0
        }
    }
